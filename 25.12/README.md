...
- **Boot layout:** kernel on `mmcblk0p1` (formatted **ext2** — U-Boot 2012.07 has no ext4load), rootfs squashfs on `mmcblk0p5` (`root=/dev/mmcblk0p5`, DTS). **CORRECTION (2026-09-13): `saveenv` is NOT reliable on this board's 2012.07 U-Boot** (found to leave a damaged / non-persisting environment). The authoritative boot environment is written to `0:appsblenv` (mtd9) through the Linux NOR driver and maintained privately, never `saveenv`. (The `bootcmd='run bootp1; run bootemmc'` env string shown in earlier versions here is historical and superseded.) **`p3`/`p7` is now a dedicated recovery system** (RAM overlay, key-only SSH, no auto-mounts), not a passive fallback.
...
- **Install / recovery:** the safe, guarded install + A/B recovery tooling is maintained **privately** (it embeds SSH host keys). The `scripts/unit2_p5_flash_{A,B}.py` in this repo are **reference only, NOT a safe installer** — Phase A proceeds to format/write even after a checksum mismatch is merely printed, and Phase B's `saveenv` persistence was found unreliable. **Do not run them to install.** (sysupgrade is also broken on this device — image-metadata mismatch.)

## 2026-09-13 correction — MMC DMA panic fix + boot-env method

- **`patches/0994-mmci-safe-dma-fallback.patch`** (added): fixes a kernel panic on the MMC DMA error path. On a DMA transfer error `mmci-pl18x` called `dma_release_channel()` from **hard-IRQ context**; on IPQ806x BAM that reaches a coherent-memory `vunmap` that BUGs in atomic context. The patch instead sets a `disabled` flag and falls back to PIO, retaining the channels for normal teardown. DMA stays enabled until this error occurs. Fault-injection-verified on eMMC and SD (no panic). Local candidate, not upstream-approved.
- **Boot environment:** `saveenv` on this box is unreliable; repair `0:appsblenv` via Linux NOR writes and verify by re-reading. Do not rely on the Phase B `saveenv` flow.
- The intermittent **SD-slot** DMA *transfer errors* (`12180000.mmc`) are non-fatal and not yet root-caused; 0994 removes only the crash they could trigger.
- A guarded A/B update + dedicated-recovery framework (build/install/env scripts, recovery images) exists but is **maintained privately** — its recovery images embed SSH host keys, so they are intentionally not in this public repo.
